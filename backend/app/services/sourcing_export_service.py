...
async def build_sourcing_export_csv(db: AsyncSession) -> str:
    ...
    renewal_license_ids = [item.renewal_for_license_id for item in items if item.renewal_for_license_id is not None]
    predecessor_map: dict[int, License] = {}
    if renewal_license_ids:
        pred_result = await db.execute(select(License).where(License.id.in_(renewal_license_ids)))
    ...
    for item in items:
        pred = predecessor_map.get(item.renewal_for_license_id) if item.renewal_for_license_id else None
        writer.writerow(
            safe_csv_row(
                [
                    item.id,
                    ...
                    item.license_type.value if item.license_type else "",
                    item.quantity or "",
