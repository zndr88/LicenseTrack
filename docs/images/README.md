...
Drop the following image files here. They are referenced from the top-level
`README.md`. Filenames must match exactly (lowercase, `.png`).
...
- Compress before committing (aim for well under ~500 KB each); git stores every
  revision of a binary in full.
- Use a consistent window width and theme across all shots.
- Avoid real customer or personal data in the captured screens.
