    ...
    sections: [
      {
        heading: "Flow",
        body: [
          "Sourcing requests can hold one or more license lines, documents, supplier details, notes, and renewal context.",
          ...
          "Perpetual, OEM, Freeware / Open Source, subscription, and SaaS lines can classify support as included. For subscription and SaaS lines, included support uses the same dates and total value as the subscription. Separately tracked support creates or links a maintenance line with its own procurement lifecycle and is only available for perpetual, OEM, or Freeware / Open Source parents.",
          "Service and Other lines have a Renewable? option, off by default. A renewable line renews like a subscription; a one-off line is retired automatically after its end date. Other lines also need a short Type Description, such as Training voucher.",
          "When a purchase is ready for procurement tracking, convert the sourcing request to a pending order. A pending order can start with a real PO number, a procurement reference, or only its Pending Order ID while the formal PO is still being created.",
          "A pending order cannot become active licenses until a real PO number is recorded. Pending orders keep the PO-level context and editable line items until the order is fulfilled.",
          "When a quote gives only a total, enter it as PO total (manual) in Edit Pending Order. This needs every line in one currency. Line prices are not changed, and the total is never spread across lines. Pending Orders marks the value Override, and every license converted from the order shows it as its Total PO Value.",
          ...
          "Sourcing Overview requests start collapsed; expand a request to review its lines. History buttons on Sourcing Overview and Pending Orders open second, read-only tables below the active work. Converted and cancelled records stay searchable there without cluttering the live pipeline.",
          "Edit Sourcing Request updates the supplier, contact, notes, and all open line items in one atomic save; converted and cancelled lines remain read-only. Documents of any category can be added and reviewed in the Documents section.",
          "When you change the supplier in Edit Sourcing Request or Edit Pending Order and a supplier contact is set, LicenseTrack asks whether to update the contact. Leaving it empty is a valid answer.",
      ...
      },
      {
        ...
        bullets: [
          "Use Documents in the sourcing or pending-order action menu to open the shared document workspace. Upload, preview, download, and delete documents there when permitted.",
          ...
          "Procurement Reference is optional workflow metadata for an internal request or approval number. It can help track an order before the formal PO number exists.",
          ...
          "History tables are paginated. Record fields are read-only there, while the Documents action keeps workflow evidence available.",
          "Maintenance lines require an explicit perpetual, OEM, or freeware/open-source parent during conversion. A maintenance line can later be linked to additional eligible parents from License Details.",
          ...
          "Est. Line Total is the estimate for one line. If it differs from quantity x unit price, the form shows a hint with the calculated value; the hint does not block saving.",
          "Copy shared fields copies only non-empty values from the first line, including secondary contacts, and reports how many fields it copied to how many lines. It never blanks a value on another line.",
    ...
    sections: [
      {
        ...
        body: [
          ...
          "An unhandled notice deadline inside the window also brings a license into the workbench, even when its end date is further out. Rows are ordered by whichever deadline comes first. When the notice date is earlier, the Due Date column shows it with an N marker, and the row is flagged Notice deadline in N days. The Notice Due view lists these rows.",
          "Included support on a perpetual, OEM, or Freeware / Open Source license gets its own workbench row, marked (included support), when the support end date is inside the window. Start support renewal creates a sourcing request with one maintenance line that already names the supported license, starting the day after the included support ends. Record existing support opens the ordinary maintenance form for support that was already bought. Coverage stays Included until the new maintenance record exists; activating it keeps the included period in Coverage History.",
      ...
      },
      {
        ...
        bullets: [
          ...
          "For separate terms purchased on one PO, edit a sourcing line and use the Term succession section near the bottom to add a next term or select existing request lines as its predecessors. Multiple earlier lines can feed one combined future term. The links stay with the lines through Pending Orders and become license renewal links when the whole PO is converted.",
          "A linked future term is Upcoming until its start date. Its predecessor no longer needs a separate renewal action. At conversion, invoices for these multi-term purchases default to Single so each year's invoice can be attached to its own term; the scope remains editable.",
          "Maintenance terms can be planned the same way, in chains of any length (year 1, year 2, year 3, and so on). Maintenance terms follow maintenance terms only. A later term without its own parent supports the same license as the term before it. Each term becomes the parent's active support on its own start date: a daily check hands over coverage, and the previous period stays in Coverage History.",
          "Service and Other records take part in renewals only when marked Renewable. One-off Service and Other records never appear in the workbench, never raise expiry alerts, and are retired automatically after their end date.",
          "High-value flags compare each row with the threshold set for its own currency. No currency conversion is applied, and a currency without a threshold is never flagged.",
          "The renewal area in License Details can be hidden with its close button. It returns when you reopen the license or open another one. The Term chain for a renewal chain sits in the History section and starts collapsed.",
          ...
          "Estimated annual value in the Renewal Workbench annualizes a multi-year term rather than showing the complete term cost as one year. A one-year term counts up to 366 days, covering leap years and terms that end on the anniversary date, so it is never scaled down.",
        ],
      },
    ],
  },
    ...
    sections: [
      {
        ...
        body: [
          ...
          "Sortable headers use the value shown by the column: dates and timestamps follow chronology, PO Total uses the whole PO value, Expiration follows lifecycle urgency, and typed custom fields use their number, date, boolean, or text meaning. Headers without a supported value remain draggable but do not offer sorting.",
          ...
          "Add License opens Review License Data. You can add several license lines, configure included support on supported lines, configure separately tracked support on eligible parent lines, and save the complete batch in one operation. If any line fails validation, none of the licenses are created. Choosing a license type applies its default Maintenance / Support Coverage, so a new subscription or SaaS line starts as Included.",
          "The browser address reflects the page and the selected license, for example /licenses/123. Back and Forward move between pages, a copied address opens the same page or license for a signed-in user who can see it, and after signing in you return to the address you asked for. Opening a License Details section adds its name to the address, for example /licenses/123#documents, so a shared link opens that section too. The names are key-dates, details, maintenance, relationships, documents, completeness, notes, custom-fields, and history.",
        ],
      },
      {
        ...
        bullets: [
          ...
          "Type: the license model, such as subscription, perpetual, maintenance, SaaS, OEM, freeware, service, or other. Service is useful for installation, implementation, or managed services; Other keeps uncommon purchases visible and needs a short Type Description. Service and Other are one-off purchases unless Renewable? is ticked: a renewable one renews and counts toward recurring cost like a subscription, and a one-off one is retired automatically after its end date.",
          ...
          "Unit Price, Total PO Value, and Currency: retain acquisition values. Unit Price is line-level acquisition pricing, and Line Total is the per-line total (quantity x unit price by default, used as a fallback value when either is missing); Total PO Value is computed from license rows sharing a PO number unless an editor or admin sets one shared manual override from License Details. Freeware/open-source acquisition pricing is zero, while paid support remains separately visible under Maintenance / Support.",
          "Start Date and End Date: the active entitlement period. Perpetual, OEM, and Freeware / Open Source licenses never carry an end date; Service and Other may have one.",
          "Notice Date: an optional manually entered contractual notice deadline, such as the last date to cancel or change renewal terms. It is not calculated from the end date, and can be marked handled after review.",
      ...
      },
      {
        ...
        bullets: [
          ...
          "Created: when the license record itself was created in LicenseTrack. The overview column shows only the date; License Details history keeps the full timestamp. This differs from Request Date and Purchase Date: those fields preserve the earlier procurement history.",
          ...
          "Maintenance / Support Coverage: classifies support as unknown, not applicable, included, or separately tracked. Included support stays on the parent; separately tracked support uses a linked maintenance license and is limited to perpetual, OEM, or Freeware / Open Source parents.",
          "Maintenance Start and End: identify the support coverage period. For separately tracked support, the parent mirrors the active maintenance license's dates. For subscription and SaaS records with included support, support dates are derived from the subscription dates.",
          "Maintenance Pricing Basis, Covered Quantity, Support Unit Price, and Support Cost: describe included support as one flat coverage fee, a per-unit calculation, or Free (no charge). Free stores a zero support cost and hides the cost fields; the record counts as priced but adds nothing to support spend. For subscription and SaaS records with included support, Support Cost is derived from the subscription acquisition total and the derived fields are hidden in create and conversion forms.",
          "Support status: for perpetual, OEM, and Freeware / Open Source records with included support and a support end date, support is active, expiring (inside the expiry alert window), or expired. Expiring and expired support show a badge in the registry and in the Maintenance section, and the Support due filter lists them.",
          ...
          "PO Total Override: an optional manual whole-PO value used when an invoice has one total but no usable line breakdown. It is shared by every license from the same pending order (or, for licenses not created from one, the same PO number) in the same currency, and can be set or cleared from any of those records. A manual PO total entered on a pending order becomes this override on every license converted from it.",
      ...
      },
      {
        heading: "Things to know",
        bullets: [
          "Perpetual, OEM, Freeware / Open Source, and one-off Service or Other records are not incomplete for missing an end date. A renewable Service or Other record needs one.",
          "An OEM or Freeware / Open Source record saved with an end date before 1.1.24 keeps it until the record is edited; saving it in the full Edit form then clears the end date.",
          ...
          "Annual cost totals are active recurring-cost rollups: active, perpetual-status, and expiring subscription, SaaS, maintenance, renewable Service and Other, and current paid included-support costs are included; multi-year records are annualized, and selected report ranges allocate recurring cost by overlapping days. Zero-cost freeware, one-off Service and Other, and upcoming, expired, retired, renewed, and legacy records are excluded.",
          "Annual cost is calculated from license lines, so a manual PO total is not reflected in it. When that matters, the License Overview header and Reports note how many POs have a manual total not reflected there.",
          ...
          "Click the Invoice # value or add control in Key Dates & Contract to manage multiple invoice numbers. Reorder the list to choose which invoice appears as the primary invoice in the Registry and exports. The full Edit form lists every invoice number too, so saving it keeps them all.",
          "The full Edit form offers the Maintenance type only on records that are already maintenance. Add new maintenance from the Maintenance section of the license it supports.",
          "For perpetual, OEM, and Freeware / Open Source records with Included coverage, Edit support in the Maintenance section changes the included support period, pricing basis, and cost. The cost is optional. Records whose support is tracked on a linked maintenance record are edited through that record instead.",
          ...
          "Inline editing is for quick field corrections; use the detail panel for full review. Supplier Contact, Budget Owner, Currency, Purchase Date, and Maintenance / Support Coverage can be edited inline. Portal URL is editable inline on SaaS records, and Invoice # only while a license has a single invoice number; with several, the cell shows a count and the list is edited in License Details.",
          ...
          "Disabling a shared maintenance link clears the selected parent only. The maintenance record remains active while it is still linked to another parent.",
          "Coverage History preserves prior included-support and maintenance-contract periods when a new active record replaces them. Retired maintenance records remain historical and cannot be selected for a new active link.",
          "Deleting a license permanently removes its license-owned documents. Active renewal or procurement relationships still block deletion; cancelled renewal history is retained with the deleted predecessor reference removed. Shared maintenance is reassigned to a remaining parent, or retired when no parent remains.",
        ],
      },
    ],
  },
    ...
    sections: [
      {
        ...
        body: [
          ...
          "Preview columns can be hidden temporarily for easier review. Unresolved maintenance parents use a searchable picker, and the collapsible reference-data review supports bulk decisions for possible duplicates before individual exceptions are handled.",
        ],
      },
      {
        heading: "Things to know",
        bullets: [
          ...
          "CSV files exported from LicenseTrack can be adjusted in Excel and re-imported. The importer tolerates common Excel rewrites such as changed quoting, delimiter hints, and localized number formatting when the correct import number format is selected.",
          ...
          "Renewable and Type Description are exported and importable. Renewable accepts Yes or No and is kept only for Service and Other rows; a blank cell leaves the row one-off. An Other row without a Type Description imports with a warning to add one later.",
          "The Contact Email column is the Supplier Contact. Supplier Contact and the older Publisher Contact header both import into it.",
          ...
          "An included-support end date in the past produces an acknowledgement-required expired-maintenance warning; it does not by itself classify a perpetual, OEM, or freeware parent as legacy.",
    ...
    sections: [
      {
        ...
        body: [
          ...
          "The date-range filter includes license records whose terms overlap the selected range. Filters apply across spend, publisher and vendor overview, portfolio health, forecast, and renewal calendar.",
      ...
      },
      {
        ...
        bullets: [
          ...
          "Purchase Order Value Tracker shows each PO and currency with its authoritative PO value, priced license-line value, and difference. Manual overrides are used once; lines without a PO remain individually counted.",
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          "New installations require PO number, invoice number, and budget owner for completeness. Other requirements are opt-in, and admins can change any of them. Upgrading never changes the stored requirements of an existing installation.",
          "Renewals settings hold one high-value threshold per currency. Your display currency and every currency with a threshold are shown; the rest sit behind Show other currencies. Leave a currency blank to never flag it. Upgrading copies an earlier single threshold to the first admin's display currency, or EUR when none is set.",
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          "One-off Service and Other records do not raise expiring or expired alerts; they are retired automatically after their end date.",
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          "Procurement reference: an optional internal request, approval, or purchasing-process number that can be tracked before or alongside a formal PO number.",
          "Supplier contact: whoever you bought from - the reseller, or the publisher for a direct purchase.",
          "Included support: support bought as part of a license rather than as a separate maintenance record. On perpetual, OEM, and Freeware / Open Source licenses it has its own end date and support status.",
          "Renewable Service / Other: a Service or Other record marked to renew like a subscription. Without the mark it is a one-off purchase.",
          "Manual PO total: a total entered for a whole pending order or PO when the quote has no usable line prices. It is shown as the Total PO Value and is never spread across lines.",
      ...
      },
      {
        ...
        bullets: [
          ...
          "Pending order: purchase-stage procurement parent. It can exist before the formal PO number is known, but final conversion to active licenses requires that PO number.",
