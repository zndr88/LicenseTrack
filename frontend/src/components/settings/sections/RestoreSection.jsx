...
export default function RestoreSection({ isOpen, isDirty, onToggle, onError, onToast }) {
  ...
  return (
    <>
      ...
      {showRestoreConfirm && (
        <ConfirmDialog title="Restore Database" message={`Restore the database from "${restoreFile?.name}"? The current database will be overwritten and the server will restart. Uploaded document files are not included in database restore. This cannot be undone.`} confirmLabel="Restore Database" danger onConfirm={handleRestore} onCancel={() => setShowRestoreConfirm(false)} />
      )}
    </>
  );
}
