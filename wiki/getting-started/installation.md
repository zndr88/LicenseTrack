...
!!! note "Always use the current latest version"
    The commands here use `v1.1.22`. Check the [latest release](https://github.com/zndr88/LicenseTrack/releases/latest) and substitute its version tag throughout. Some screenshots on this page were captured on an earlier version — the look and feel is unchanged, only the version number differs.
